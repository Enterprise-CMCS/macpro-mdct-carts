...
Cypress.Commands.add('displaysSectionsWhenUserIsReporting', () => {
  ...
  cy.get('[data-cy="Why are you not reporting on this measure?"]').should('not.exist');
  ...
  cy.get('[data-cy="Definition of Population Included in the Measure"]').should('be.visible');
  cy.get('[data-cy="Combined Rate(s) from Multiple Reporting Units"]').should('be.visible');
  cy.get('[data-cy="Additional Notes/Comments on the measure (optional)"]').should('be.visible');
});
...
Cypress.Commands.add('displaysSectionsWhenUserNotReporting', () => {
  ...
  cy.get('[data-cy="Definition of Population Included in the Measure"]').should('not.exist');
  cy.get('[data-cy="Combined Rate(s) from Multiple Reporting Units"]').should('not.exist');

  // these sections should be visible when a user selects they are not reporting
  cy.get('[data-cy="Why are you not reporting on this measure?"]').should('be.visible');
  cy.get('[data-cy="Additional Notes/Comments on the measure (optional)"]').should('be.visible');
});
...
Cypress.Commands.add('deleteChildCoreSets', () => {
  cy.get('tbody').then(($tbody) => {
    if ($tbody.find('[data-cy="child-kebab-menu"]').length > 0) {
      cy.get(':nth-child(2) > :nth-child(5) > .css-xi606m > [data-cy="child-kebab-menu"]').click({
        force: true,
      });
      cy.xpath(
