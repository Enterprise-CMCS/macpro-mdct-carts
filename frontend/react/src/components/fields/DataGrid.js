...
import lastYearFormData from "../../store/lastYearFormData";
...
const DataGrid = ({question, state}) => {
    ...
    const getDataFromLastYear = (item) => {
        ...
        if (
            ...
            if (lastYearFormData) {
                console.log("DEBUG: ZAC : xxxxxxxxx " + lastYear)
                // If data exists, decipher value from return result (data)
                let data = lastYearFormData[0]
                let prevYearValue =
                    parseInt(
                        (data, fieldsetId, questionId)
                    ) || "";
                ...
                console.log("ZAC DEBUG:" + JSON.stringify(questionsToSet))
                console.log("DEBUG: ZAC : GFGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGggg")
