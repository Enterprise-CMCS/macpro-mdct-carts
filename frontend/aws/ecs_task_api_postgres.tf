locals {
  ...
  prince_api_ssm_path = substr(terraform.workspace, 0, 4) == "acti" ? "/default/prince_api_endpoint"  : "/${terraform.workspace}/prince_api_endpoint"
}


data "aws_ssm_parameter" "prince_api_endpoint" {
  name = local.prince_api_ssm_path
}
