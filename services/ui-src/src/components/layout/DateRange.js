...
class DateRange extends Component {
  ...
  checkChronology() {
    ...
    const { yearStart, yearEnd, startErrorMessage, endErrorMessage } = this.state;
    ...
    if (monthStart && monthEnd && yearStart && yearEnd) {
      ...
      const payload = [`${yearStart}-${monthStart}-01`, `${yearEnd}-${monthEnd}-01`];
  ...
  validateYear(input) {
    ...
    if (
      ...
      // failing = true;
      returnString = 'Please enter a number';
    } else if (parseInt(input, 10) < 1776 || parseInt(input, 10) > parseInt(year, 10)) {
      // Checks that the month value is within a normal range
    ...
    return (
      <div className="date-range" data-test="component-date-range">
        ...
        <div className="date-range-start">
          ...
          <div className="errors">
            {endRangeErr === true ? <div> End date must come after start date</div> : null}
          </div>
        </div>
      </div>
    );
