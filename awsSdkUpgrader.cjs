...
const result = execSync("cd services/app-api && yarn info @aws-sdk/client-dynamodb version && cd -").toString();
const latestSdkVersionNumber = result.split("\n")[1].trim();
const versionMatch = /^3\.(\d+)\.0$/.exec(latestSdkVersionNumber);
if (!versionMatch) {
  throw new Error(`Unexpected @aws-sdk version number: ${latestSdkVersionNumber}`);
}
...
for (let directory of serviceDirectories) {
  const service = JSON.parse(readFileSync(path.join(directory, "package.json"), "utf8"));
...
execSync(`git commit -am 'Bump AWS SDK version numbers to ${latestSdkVersionNumber}'`);
execSync(`git push`);
